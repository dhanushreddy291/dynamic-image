...
const STORAGE_URL =
  "https://obuldanrptloktxcffvn.supabase.co/storage/v1/object/public/images/lw6";
...
const font = fetch(new URL(FONT_URL, import.meta.url)).then((res) =>
  res.arrayBuffer()
);
...
export async function handler(req: Request) {
  ...
  const username =
    url.searchParams.get("username") ?? url.searchParams.get("amp;username");
  const name = url.searchParams.get("name") ?? url.searchParams.get("amp;name");
  const golden =
    url.searchParams.get("golden") ?? url.searchParams.get("amp;golden");
  ...
  try {
    ...
    const storageResponse = await fetch(
      `${STORAGE_URL}/tickets/${username}.png?v=3`
    );
    ...
    const generatedImage = new ImageResponse(
              ...
              >
                ...
                >
                  <img
                    width="32"
                    height="32"
                    src={golden ? SUPA_CHECKMARK_GOLD : SUPA_CHECKMARK}
                  />
                </span>
              </div>
            </div>
      ...
      ),
      {
        ...
        headers: {
          ...
          "cache-control":
            "public, max-age=31536000, s-maxage=31536000, no-transform, immutable",
